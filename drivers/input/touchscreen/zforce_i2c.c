	...
	for (n = 0; n < count; n++) {
		...
		if (2 == state) {
			...
			if (id==0) {
				...
				input_report_abs(zForce_ir_touch_data.input, ABS_PRESSURE, gIsCustomerUi ? pressure : 1024);	// Joseph 20101023
			}
		}
	}
...
static void zForce_ir_touch_work_func(struct work_struct *work)
{
 ...
 if (zForce_ir_touch_detect_int_level () && g_touch_pressed[0]) {
  ...
  if (200 > ++g_touch_pressed_timer) {
   ...
   zForce_ir_touch_report_data(zForce_ir_touch_data.client, gzForceBuffer[gQueueFront]);
	...
	if (g_touch_pressed[0] ||!zForce_ir_touch_detect_int_level ())
		schedule_delayed_work(&zForce_ir_touch_data.work, 1);
	else
		g_touch_triggered = 0;
}
